...
const Home = () => {
  ...
  useEffect(() => {
    // Only fetch events if user is logged in (optional check, but good practice)
    const user = localStorage.getItem("user");
    if (user) {
      AxiosInstance.get("/event/Eventlist")
        .then((res) => {
          if (res.data.EventList) {
            const today = new Date();
            const upcomingEvents = res.data.EventList.filter(
